...
%\section{Essential Properties} We should call it this if we have more properties to put here

Consider the following property of a complexity class $C$:
\begin{definition}\label{defintersection}\emph{
We say that a class $C$ is \emph{closed under intersection} if for any languages $L_1,L_2,...,L_k \in C$, the language $\{(x_1,...,x_k) : x_i \in L_i ~ \forall i\}$ is in $C$.
}\end{definition}
It is obvious that a class such as $\p$ is closed under intersection. We will now show that the operators $\Sigma$, $\Pi$, $\bp$, and $\parity$ all preserve this property, which will be important in Section \ref{oracle}
\begin{theorem}\label{theoremintersection}
